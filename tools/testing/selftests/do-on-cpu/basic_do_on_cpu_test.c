...
static int test_compare_eq_len_op(char *a, char *b, size_t len)
{
	...
	enum {
		BPF_LABEL_LOOP = 6,
		BPF_LABEL_BRANCH1 = 7,
		BPF_LABEL_BRANCH2 = 15,
	};

	struct bpf_insn bytecode[] = {
		[0] = BPFI_LD_IMM64(BPF_REG_2, BPF_PTR_TO_V(a)),
		[2] = BPFI_LD_IMM64(BPF_REG_3, BPF_PTR_TO_V(b)),
		[4] = BPFI_LD_IMM64(BPF_REG_4, len),

		/* Loop target. */
		[BPF_LABEL_LOOP] = BPFI_JEQ_K(BPF_REG_4, 0,
					      BPF_LABEL_BRANCH2 - BPF_LABEL_BRANCH1),

		[BPF_LABEL_BRANCH1] = BPFI_LDX(BPF_B, BPF_REG_0, BPF_REG_2, 0),
		[8] = BPFI_LDX(BPF_B, BPF_REG_1, BPF_REG_3, 0),
		[9] = BPFI_SUB64_X(BPF_REG_0, BPF_REG_1),
		[10] = BPFI_JNE_K(BPF_REG_0, 0, 4),

		/* Same, test next byte. */
		[11] = BPFI_ADD64_K(BPF_REG_2, 1),
		[12] = BPFI_ADD64_K(BPF_REG_3, 1),
		[13] = BPFI_SUB64_K(BPF_REG_4, 1),
		[14] = BPFI_JA_K(BPF_LABEL_LOOP - BPF_LABEL_BRANCH2),

		/* Completed. BPF_REG_0 contains the return value. */
		[BPF_LABEL_BRANCH2] = BPFI_EXIT(),
	};
...
static int test_memcpy_len_op(char *dst, char *src, size_t len)
{
	...
	struct bpf_insn bytecode[] = {
		...
		[BPF_LABEL_BRANCH1] = BPFI_LDX(BPF_B, BPF_REG_1, BPF_REG_3, 0),
		[8] = BPFI_STX(BPF_B, BPF_REG_2, BPF_REG_1, 0),

		[9] = BPFI_ADD64_K(BPF_REG_2, 1),
		[10] = BPFI_ADD64_K(BPF_REG_3, 1),
		[11] = BPFI_SUB64_K(BPF_REG_4, 1),
		[12] = BPFI_JA_K(BPF_LABEL_LOOP - BPF_LABEL_BRANCH2),
		...
		void *dst2, void *src2, size_t len2)
{
	...
	struct bpf_insn bytecode[] = {
		...
		[4] = BPFI_LD_IMM64(BPF_REG_4, len1),
		...
		[BPF_LABEL_BRANCH2] = BPFI_LD_IMM64(BPF_REG_0, BPF_PTR_TO_V(dst2)),
		[15] = BPFI_LD_IMM64(BPF_REG_1, BPF_PTR_TO_V(src2)),
		[17] = BPFI_LDX(bpf_size2, BPF_REG_1, BPF_REG_1, 0),
		[18] = BPFI_STX_RELEASE(bpf_size2, BPF_REG_0, BPF_REG_1, 0),
