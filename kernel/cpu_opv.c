 ...
 * change between preparation of system call inputs and interpretation
 * of the operation vector within the kernel.
 *
 ...
 * too long interrupt-off critical section or IPI handler (cache cold
 * critical section duration measured as 4.7µs on x86-64). Each operation
 * is also limited a length of 4096 bytes, meaning that an operation can
 * touch a maximum of 4 pages (memcpy: 2 pages for source, 2 pages for
