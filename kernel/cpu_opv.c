				...
				bool *expect_fault)
{
	...
	case CPU_COMPARE_NE_OP:
		...
		*expect_fault = op->u.compare_op.expect_fault_a;
		if (!access_ok((void __user *)(unsigned long)op->u.compare_op.a,
			       op->len))
			return ret;
		...
		*expect_fault = op->u.compare_op.expect_fault_b;
		if (!access_ok((void __user *)(unsigned long)op->u.compare_op.b,
			       op->len))
			return ret;
	...
	case CPU_MEMCPY_RELEASE_OP:
		...
		*expect_fault = op->u.memcpy_op.expect_fault_dst;
		if (!access_ok((void __user *)(unsigned long)op->u.memcpy_op.dst,
			       op->len))
			return ret;
		...
		*expect_fault = op->u.memcpy_op.expect_fault_src;
		if (!access_ok((void __user *)(unsigned long)op->u.memcpy_op.src,
			       op->len))
			return ret;
	...
	case CPU_ADD_RELEASE_OP:
		...
		*expect_fault = op->u.arithmetic_op.expect_fault_p;
		if (!access_ok((void __user *)(unsigned long)op->u.arithmetic_op.p,
			       op->len))
			return ret;
