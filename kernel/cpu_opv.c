...
static int cpu_op_check_page(struct page *page, int write)
{
	 ...
	 * but it might be the ZERO_PAGE (which is OK to read from), or in
	 * the gate area or in a special mapping (in which cases we are
	 * happy to fail); or it may have been a good file page when
	 * get_user_pages_fast found it, but truncated or holepunched or
	 * subjected to invalidate_complete_page2 before we got the page
	 * lock (also cases which we are happy to fail). And we hold a
	 * reference, so refcount care in invalidate_complete_page's
	 * remove_mapping prevents drop_caches from setting mapping to
	 * NULL beneath us.
	 *
...
{
	switch (len) {
	case 1:	data->_u8 = READ_ONCE(*(uint8_t *)p);
		break;
	case 2:	data->_u16 = READ_ONCE(*(uint16_t *)p);
		break;
	case 4:	data->_u32 = READ_ONCE(*(uint32_t *)p);
		break;
	case 8:
#if (BITS_PER_LONG == 64)
...
{
	switch (len) {
	case 1:	WRITE_ONCE(*(uint8_t *)p, data->_u8);
		break;
	case 2:	WRITE_ONCE(*(uint16_t *)p, data->_u16);
		break;
	case 4:	WRITE_ONCE(*(uint32_t *)p, data->_u32);
		break;
	case 8:
#if (BITS_PER_LONG == 64)
