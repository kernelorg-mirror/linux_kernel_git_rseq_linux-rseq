				       ...
				       size_t len, enum do_on_cpu_rw rw)
{
	...
	size_t nr_pages = map->nr_pages;
	size_t nr_vmaps = map->nr_vmaps;
	...
	for (i = 0; i < nr_pages; i++) {
		...
		enum do_on_cpu_rw page_rw = doc_page->rw;
		unsigned long page_base = doc_page->uaddr_base;

		if (unlikely(page_rw != DO_ON_CPU_WRITE && rw != DO_ON_CPU_READ))
			continue;
		if (!found_start && page_base == (uaddr & PAGE_MASK)) {
			found_start = true;
			doc_page->used = 1;
		}
		if (!found_end &&
		    page_base == ((uaddr + len - 1) & PAGE_MASK)) {
			found_end = true;
			doc_page->used = 1;
		}
	...
	if (unlikely(!found_start || !found_end))
		return 0;

	/* Find kaddr. */
	for (i = 0; i < nr_vmaps; i++) {
		struct do_on_cpu_vmap *doc_vmap = &map->vmap[i];
		unsigned long vmap_uaddr_base = doc_vmap->uaddr_base;

		if (uaddr >= vmap_uaddr_base &&
		    uaddr + len <= doc_vmap->uaddr_end) {
			doc_vmap->used = 1;
			return uaddr - vmap_uaddr_base + doc_vmap->kaddr_base;
		}
	}
	return 0;
}
		    ...
		    enum on_cpu_pass pass)
{
	...
	case ON_CPU_PASS_PIN_PAGES:
		...
		if (tainted) {
			...
			if (unlikely(ctx->nr_tainted_mem_ops[
	...
	case ON_CPU_PASS_STORES:
		if (tainted) {
			...
			if (ctx->nr_stores > 0)
				...
				ret = -EAGAIN;
			if (unlikely(ctx->nr_tainted_mem_ops[ON_CPU_PASS_STORES] >=
			    ctx->nr_tainted_mem_ops[ON_CPU_PASS_PIN_PAGES]))
				return ret;
			mem_op = &ctx->tainted_mem_ops[ctx->nr_tainted_mem_ops[
						       ON_CPU_PASS_STORES]++];
			if (unlikely(mem_op->addr != ptr || mem_op->len != len ||
			    mem_op->rw != DO_ON_CPU_READ)) {
				return ret;
			}
		}
...
int deref_uptr_store(struct reg *dst_reg, s16 off, size_t len, u64 src,
		     int release, struct do_on_cpu_map *map,
		     struct do_on_cpu_mem_op *next_pin,
		     struct do_on_cpu_ctx *ctx,
		     enum on_cpu_pass pass)
{
	      ...
	      enum on_cpu_pass pass)
{
	...
	switch (pass) {
	case ON_CPU_PASS_PIN_PAGES:
		if (unlikely(ctx->nr_tainted_branches[ON_CPU_PASS_PIN_PAGES] >=
	...
	case ON_CPU_PASS_STORES:
	{
		...
		if (ctx->nr_stores > 0)
			...
			ret = -EAGAIN;
		if (unlikely(ctx->nr_tainted_branches[ON_CPU_PASS_STORES] >=
		    ctx->nr_tainted_branches[ON_CPU_PASS_PIN_PAGES]))
			return ret;
		if (unlikely(test_bit(ctx->nr_tainted_branches[
			  ...
			  enum on_cpu_pass pass)
{
	...
	for (;;) {
		...
		insn = &bytecode[pc];
		if (unlikely(nr_interpreted_insn++ >= DO_ON_CPU_RETIRED_INSN_MAX)) {
		...
		case BPF_LDX | BPF_W | BPF_MEM:
			ret = deref_uptr_load(&reg[insn->dst_reg], &reg[insn->src_reg], insn->off,
					sizeof(u32), 0, map, next_pin, ctx, pass);
			if (unlikely(ret))
				goto error_mem;
			pc++;
			break;
		case BPF_LDX | BPF_H | BPF_MEM:
			ret = deref_uptr_load(&reg[insn->dst_reg], &reg[insn->src_reg], insn->off,
					sizeof(u16), 0, map, next_pin, ctx, pass);
			if (unlikely(ret))
				goto error_mem;
			pc++;
			break;
		case BPF_LDX | BPF_B | BPF_MEM:
			ret = deref_uptr_load(&reg[insn->dst_reg], &reg[insn->src_reg], insn->off,
					sizeof(u8), 0, map, next_pin, ctx, pass);
			if (unlikely(ret))
				goto error_mem;
			pc++;
			break;
		case BPF_LDX | BPF_DW | BPF_MEM:
			ret = deref_uptr_load(&reg[insn->dst_reg], &reg[insn->src_reg], insn->off,
					sizeof(u64), 0, map, next_pin, ctx, pass);
			if (unlikely(ret))
				goto error_mem;
			pc++;
			break;
		...
		case BPF_LDX | BPF_W | BPF_MEM_ACQ_REL:
			ret = deref_uptr_load(&reg[insn->dst_reg], &reg[insn->src_reg], insn->off,
					sizeof(u32), 1, map, next_pin, ctx, pass);
			if (unlikely(ret))
				goto error_mem;
			pc++;
			break;
		case BPF_LDX | BPF_H | BPF_MEM_ACQ_REL:
			ret = deref_uptr_load(&reg[insn->dst_reg], &reg[insn->src_reg], insn->off,
					sizeof(u16), 1, map, next_pin, ctx, pass);
			if (unlikely(ret))
				goto error_mem;
			pc++;
			break;
		case BPF_LDX | BPF_B | BPF_MEM_ACQ_REL:
			ret = deref_uptr_load(&reg[insn->dst_reg], &reg[insn->src_reg], insn->off,
					sizeof(u8), 1, map, next_pin, ctx, pass);
			if (unlikely(ret))
				goto error_mem;
			pc++;
			break;
		case BPF_LDX | BPF_DW | BPF_MEM_ACQ_REL:
			ret = deref_uptr_load(&reg[insn->dst_reg], &reg[insn->src_reg], insn->off,
					sizeof(u64), 1, map, next_pin, ctx, pass);
			if (unlikely(ret))
				goto error_mem;
			pc++;
			break;
		...
		case BPF_ALU | BPF_DIV | BPF_X:
			if (unlikely(!(u32) reg[insn->src_reg].v)) {
		...
		case BPF_ALU | BPF_LSH | BPF_X:
			if (unlikely(reg[insn->src_reg].v >= 32 || reg[insn->src_reg].v < 0)) {
		...
		case BPF_ALU | BPF_MOD | BPF_X:
			if (unlikely(reg[insn->src_reg].v == 0)) {
		...
		case BPF_ALU64 | BPF_DIV | BPF_X:
			if (unlikely(!reg[insn->src_reg].v)) {
				printk(KERN_ERR "Error: Divide by 0\n");
				ret = -EINVAL;
				goto end;
			}
		...
		case BPF_ALU64 | BPF_LSH | BPF_X:
			if (unlikely(reg[insn->src_reg].v >= 64 || reg[insn->src_reg].v < 0)) {
				printk(KERN_ERR "Error: Left shift by %lld undefined.\n",
				       reg[insn->src_reg].v);
				ret = -EINVAL;
				goto end;
	...
	/* ret values 0 and 1 are used for branch. Negative ret is an error. */
	if (ret >= 0)
		ret = 0;
	if (!ret && result)
